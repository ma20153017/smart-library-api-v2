// 智能图书馆高性能API - v3.3
// 支持PostgreSQL数据库 + Redis缓存 + AI智能推荐 + 动态作者系统
// 动态作者版本 - 2025.08.27
...
async function handleAuthors(req, res) {
  try {
    ...
    let sql = `
      WITH author_stats AS (
        SELECT 
          ...
          ARRAY_AGG(DISTINCT title ORDER BY popularity DESC LIMIT 3) as sample_books,
          CASE 
        ...
        WHERE author IS NOT NULL 
          ...
          ${search ? 'AND author ILIKE $1' : ''}
        GROUP BY author
        HAVING COUNT(*) >= $${search ? 2 : 1}
      ),
      total_count AS (
        SELECT COUNT(*) as total FROM author_stats
      )
      SELECT 
        a.*,
        t.total,
        ROUND(a.avg_popularity::numeric, 2) as avg_popularity_rounded
      FROM author_stats a, total_count t
      WHERE a.book_count >= $${search ? 3 : 2}
    `;
    
    // 添加排序
    const sortMappings = {
      'books_desc': 'ORDER BY a.book_count DESC, a.avg_popularity DESC',
      'books_asc': 'ORDER BY a.book_count ASC, a.author ASC',
      'name_asc': 'ORDER BY a.author ASC',
      'popularity_desc': 'ORDER BY a.avg_popularity DESC, a.book_count DESC',
      'diversity_desc': 'ORDER BY a.subject_diversity DESC, a.book_count DESC'
    };
    
    sql += ` ${sortMappings[sort] || sortMappings['books_desc']}`;
    sql += ` LIMIT $${search ? 4 : 3} OFFSET $${search ? 5 : 4}`;
    
    // 执行查询
    let queryParams;
    if (search) {
      queryParams = [`%${search}%`, minBooks, limitNum, offset];
    } else {
      queryParams = [minBooks, limitNum, offset];
    }
    ...
    if (authors.length === 0) {
      ...
      const totalAuthors = authors[0].total;
      ...
      const processedAuthors = authors.map(author => ({
        ...
        avgPopularity: parseFloat(author.avg_popularity_rounded) || 0,
        totalViews: author.total_views || 0,
        tier: author.author_tier,
        subjects: author.subjects ? author.subjects.split(', ') : [],
        sampleBooks: author.sample_books || [],
...
async function handleRecommend(req, res) {
  try {
    ...
    // 🔍 Step 2: 增强的关键词提取（支持作者查询）
    const keywordInfo = extractSimpleKeywords(searchQuery);
    console.log(`🔤 [V3.1] 提取结果: 类型=${keywordInfo.type}, 关键词=${keywordInfo.keywords?.join(', ')}`);
    ...
    if (keywordInfo.type === 'author') {
      // 👤 作者查询处理
      isAuthorQuery = true;
      const authorName = keywordInfo.author;
      console.log(`👤 [V3.1] 处理作者查询: ${authorName}`);
      
      candidateBooks = await queryAuthorBooks(authorName, limitNum * 2);
      
      if (candidateBooks.length > 0) {
        // 生成作者推荐信息
        const recommendInfo = generateAuthorRecommendation(authorName, candidateBooks, keywordInfo.matchInfo);
        authorInfo = recommendInfo.authorInfo;
        console.log(`📚 [V3.1] 找到作者 ${authorName} 的 ${candidateBooks.length} 本作品`);
      } else {
        console.log(`❌ [V3.1] 未找到作者 ${authorName} 的作品`);
      }
    } else {
      // 📂 类别/概念查询处理
 ...
 * 检测查询是否为作者查询
 */
function detectAuthorQuery(query) {
  console.log(`🔍 [Author] 检测作者查询: "${query}"`);
  ...
  for (const pattern of AUTHOR_PATTERNS) {
    ...
    if (match && match[1]) {
      ...
      console.log(`🎯 [Author] 模式匹配到可能的作者: "${possibleAuthor}"`);
      
      // 2. 在已知作者列表中查找
      const exactMatch = KNOWN_AUTHORS.find(author => author === possibleAuthor);
      if (exactMatch) {
        console.log(`✅ [Author] 精确匹配作者: ${exactMatch}`);
        return {
          type: 'author',
          author: exactMatch,
          matchType: 'exact',
      ...
      // 3. 模糊匹配
      const fuzzyMatch = KNOWN_AUTHORS.find(author => 
        author.includes(possibleAuthor) || possibleAuthor.includes(author)
      );
      if (fuzzyMatch) {
        console.log(`🔍 [Author] 模糊匹配作者: ${fuzzyMatch} (输入: ${possibleAuthor})`);
        return {
          type: 'author',
          author: fuzzyMatch,
          matchType: 'fuzzy',
          pattern: pattern.source,
          original: possibleAuthor
        };
      }
    }
  }
  ...
  for (const author of KNOWN_AUTHORS) {
    if (text.includes(author)) {
      console.log(`✅ [Author] 直接包含作者名: ${author}`);
      return {
        type: 'author',
        author: author,
        matchType: 'contains',
...
function extractSimpleKeywords(query) {
  ...
  // 🎯 优先检测作者查询
  const authorDetection = detectAuthorQuery(query);
  if (authorDetection) {
