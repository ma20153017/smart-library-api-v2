...
返回：{
  ...
  "explanation": "涵盖AI核心概念和相关技术领域"
    ...
    async function extractKeywords(text) {
      ...
      console.log(`🔤 [V2.3] 关键词提取，原文: "${text}"`);
      ...
      for (const [key, values] of Object.entries(simpleMapping)) {
        if (text.includes(key)) {
          ...
          found = true;
          console.log(`✅ [V2.3] 找到关键词: ${key} -> ${values.join(', ')}`);
          break;
        }
      }
      ...
      if (!found) {
        ...
        if (cleaned.length > 0) {
          ...
          // 简单繁简转换
          const traditional = cleaned.replace(/数学/g, '數學').replace(/儿童/g, '兒童').replace(/文学/g, '文學');
          if (traditional !== cleaned) {
            keywords.push(traditional);
          }
        } else {
          keywords.push('小說', '教育', '文學');
        }
        console.log(`🔍 [V2.3] 通用处理，关键词: [${keywords.join(', ')}]`);
      }
      
      console.log(`🎯 [V2.3] 最终关键词: [${keywords.join(', ')}]`);
      return keywords; // 🚨 简化：直接返回数组，不是对象
    }
