...
async function searchCandidateBooks(query, limit = 30) {
  try {
    ...
    function extractKeywords(text) {
      ...
      console.log(`🔤 [V2] 开始提取关键词，原文: "${text}"`);
      
      // 扩展的图书类别关键词映射
      const categoryMappings = {
        '小说': ['小说', '小說', '中國小說', '外國小說', '台灣小說', '香港小說', '言情', '武侠', '科幻'],
        '儿童': ['儿童', '兒童', '兒童文學', '儿童文学', '童书', '童話', '童话'],  
        '数学': ['数学', '數學', '數學教學', '数学教学', '算術', '算术'],
        '文学': ['文学', '文學', '中國文學', '外國文學', '現代文學', '古典文學'],
        '教学': ['教学', '教學', '教育', '學習', '学习'],
        '历史': ['历史', '歷史', '中國歷史', '世界歷史'],
        '编程': ['编程', '編程', '程序设计', '計算機', '编程', '程式', '开发'],
        '英语': ['英语', '英語', 'English', '英文'],
        '散文': ['散文', '随笔', '隨筆'],
        '普及读物': ['普及讀物', '普及读物', '科普'],
        '科普': ['科普', '普及讀物', '普及读物'],
      ...
      // 更灵活的关键词检测 - 不用break，允许多个匹配
      let foundKeywords = [];
      for (const [key, variations] of Object.entries(categoryMappings)) {
        if (text.includes(key)) {
          foundKeywords.push(...variations);
          console.log(`✅ [V2] 找到关键词类别: ${key} -> ${variations.join(', ')}`);
        }
      }
      ...
      if (foundKeywords.length > 0) {
        ...
        // 通用关键词提取 - 更宽松的策略  
        let cleaned = text
          .replace(/推荐|推薦|一些|几本|找|想看|需要|要|给我|帮我|的|书籍|書籍|图书|圖書|好的|优秀|经典|什么|有/g, '')
          .trim();
        ...
        // 如果还是没有关键词，返回一些默认的热门类别
        if (keywords.length === 0) {
          console.log(`📚 [V2] 使用默认关键词策略`);
          keywords.push('小說', '教育', '文學', '兒童文學'); // 基于统计数据的热门类别
        }
      }
      
      console.log(`🎯 [V2] 最终提取的关键词: [${keywords.join(', ')}]`);
      return keywords;
    }
