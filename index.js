...
async function searchCandidateBooks(query, limit = 30) {
  try {
    ...
    // 简繁体转换（把用户输入的简体转换成繁体来匹配数据库）
    const traditionalQuery = query
      .replace(/小说/g, '小說')
      .replace(/儿童/g, '兒童')
      .replace(/数学/g, '數學')
      .replace(/历史/g, '歷史')
    ...
    console.log(`🔍 [V2] 搜索候选书籍: "${query}" -> "${traditionalQuery}"`);
    ...
    const result = await dbPool.query(sql, [`%${query}%`, `%${traditionalQuery}%`, limit]);
    return result.rows;
